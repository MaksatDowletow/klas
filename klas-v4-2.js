...
function mediaMarkup(p){
  ...
  if(image) return `<button class="post-media-shell" type="button" data-post-media="${esc(p.id)}" aria-label="${esc(p.author)} tarapyndan paýlaşylan suraty aç"><img class="post-image" src="${esc(image)}" alt="${esc(p.text.slice(0,120)||'Post suraty')}" loading="lazy"></button>`;
  if(video){ return isDirectVideo(video)?`<div class="post-video-shell"><video class="post-video" controls preload="metadata" src="${esc(video)}"></video><button class="post-video-expand" type="button" data-post-media="${esc(p.id)}" aria-label="Wideony doly viewer-de aç">⛶</button></div>`:`<button class="post-video-link" type="button" data-post-media="${esc(p.id)}">🎥 Wideony viewer-de aç</button>`; }
  return '';
}
function postMediaItems(){return state.posts.flatMap(p=>{const image=safeLocalUrl(p.image,{allowDataImage:true}),video=safeLocalUrl(p.video);if(image)return[{id:`post-${p.id}`,type:'image',src:image,title:p.author||'Post suraty',description:p.text||''}];if(video)return[{id:`post-${p.id}`,type:'video',src:video,title:p.author||'Post wideosy',description:p.text||''}];return[]})}
function viewPostMedia(id){const items=postMediaItems();if(!window.KlasMediaViewer?.open(items,`post-${id}`))toast('Media viewer açylmady')}
function postHTML(p){
...
function bindPosts(root=document){
  $$('[data-post-media]',root).forEach(button=>button.onclick=()=>viewPostMedia(button.dataset.postMedia));
...
function renderPeople(){
  ...
  $('#people').innerHTML=list.map(p=>`<div class="person"><span class="presence-avatar"><img class="avatar" src="${esc(p.avatar)}" alt="${esc(p.name)}">${p.online?'<i aria-hidden="true"></i>':''}</span><div class="person-info"><b>${esc(p.name)} ${p.online?'<span class="presence-badge"><i aria-hidden="true"></i>Onlaýn</span>':''}</b><small>${esc(p.city)} · ${esc(p.job)}</small></div><div class="person-actions"><button class="mini" data-message-person="${esc(p.id)}" aria-label="${esc(p.name)} bilen habarlaş">💬</button><button class="mini" data-friend="${esc(p.id)}" aria-label="${esc(p.name)}: dostluk ýagdaýy">${personButtonText(p.status)}</button></div></div>`).join('')||'<div class="empty">Netije tapylmady.</div>';
...
function renderOnline(){const list=state.people.filter(p=>p.online);$('#onlineCount').textContent=`● ${list.length}`;$('#onlinePeople').innerHTML=list.map(p=>`<button data-online-chat="${esc(p.id)}" title="${esc(p.name)} · Onlaýn" aria-label="${esc(p.name)} bilen çat aç; häzir onlaýn"><span class="presence-avatar"><img src="${esc(p.avatar)}" alt=""><i aria-hidden="true"></i></span></button>`).join('')||'<small class="online-empty">Häzir onlaýn agza ýok</small>';$$('[data-online-chat]').forEach(b=>b.onclick=()=>startChatWithPerson(b.dataset.onlineChat));}
