...
function bindPosts(root=document){
  ...
  $$('[data-comment-toggle]',root).forEach(b=>b.onclick=()=>{$(`#comments-${CSS.escape(b.dataset.commentToggle)}`)?.classList.toggle('hidden')});
  $$('[data-comment-form]',root).forEach(form=>form.onsubmit=e=>{e.preventDefault();const input=$('input',form),text=input.value.trim();if(!text)return;const p=state.posts.find(x=>x.id===form.dataset.commentForm);p.comments.push({id:uid(),author:state.currentUser.name,avatar:state.currentUser.avatar,text,time:'häzir'});input.value='';addNotification(`${state.currentUser.shortName} täze teswir goşdy`,'💬','feed');save();renderAllPosts();setTimeout(()=>$(`#comments-${CSS.escape(p.id)}`)?.classList.remove('hidden'),0);toast('Teswir goşuldy')});
