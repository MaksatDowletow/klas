...
const msg=t.closest('[data-message-person]');if(msg){const p=bridge.getPerson(msg.dataset.messagePerson);if(p?.remote){e.preventDefault();e.stopImmediatePropagation();const id=await ensureConversation(p.uid||p.id);bridge.setActiveChat(id);showPage('messages');setMobileChatOpen(true);return}}
