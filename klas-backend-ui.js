...
function updateAccountPanel(){
  ...
  text.textContent=incomplete
    ...
    : runtime.authPersistence==='local'
      ...
      : `${runtime.user.email||'Google akkaunty'} · brauzer sessiýany hemişelik saklamagy çäklendirýär.`;
...
function authDialog(){
  modal({
    ...
    onConfirm:async button=>{
      ...
      button.textContent='Google we Klas barlanýar…';
      try{
        await login();
        bridge.closeModal();
      }catch(error){
...
function profileEditor(){const p=runtime.profile||{};modal({title:'Firebase profili',confirmText:'Sakla',body:`<div class="form-grid"><div class="field"><label>Doly ady</label><input id="rpName" maxlength="100" autocomplete="name" value="${safe(p.fullName||runtime.user.displayName||'')}"></div><div class="form-grid two"><div class="field"><label>Şäher</label><input id="rpCity" maxlength="80" autocomplete="address-level2" value="${safe(p.city||'')}"></div><div class="field"><label>Hünär</label><input id="rpJob" maxlength="80" value="${safe(p.profession||'')}"></div></div><div class="field"><label>Bio</label><textarea id="rpBio" maxlength="500">${safe(p.bio||'')}</textarea></div><div class="field"><label>Avatar faýly</label><input id="rpFile" type="file" accept="image/*"></div><div class="field"><label>Ýa-da URL</label><input id="rpUrl" type="url" maxlength="2000" value="${safe(p.avatarURL||runtime.user.photoURL||'')}"></div></div>`,onConfirm:async b=>{const fullName=$('#rpName').value.trim();if(!fullName)throw new Error('Adyňyzy ýazyň.');b.disabled=true;try{const f=$('#rpFile').files[0];let avatarURL=$('#rpUrl').value.trim();if(f)avatarURL=await uploadMedia(f,'klas/avatars');await saveProfile({fullName,shortName:fullName.split(/\s+/)[0],city:$('#rpCity').value.trim(),profession:$('#rpJob').value.trim(),bio:$('#rpBio').value.trim(),avatarURL});bridge.closeModal();toast('Profil saklandy')}finally{b.disabled=false}}})}
...
const msg=t.closest('[data-message-person],[data-online-chat],[data-search-person],[data-chat-choice]');if(msg){const personId=msg.dataset.messagePerson||msg.dataset.onlineChat||msg.dataset.searchPerson||msg.dataset.chatChoice,p=bridge.getPerson(personId);if(p?.remote){e.preventDefault();e.stopImmediatePropagation();msg.disabled=true;msg.setAttribute('aria-busy','true');try{const id=await ensureConversation(p.uid||p.id);bridge.setActiveChat(id);if(msg.dataset.chatChoice)bridge.closeModal();const search=$('#searchResults');if(search){search.classList.add('hidden');$('#searchInput')?.setAttribute('aria-expanded','false')}showPage('messages');setMobileChatOpen(true)}catch(error){const code=String(error?.code||'');toast(code.includes('permission-denied')?'Çat açylmady. Iki hasabyň hem işjeň we doly tamamlanandygyny barlaň.':error?.message||'Çat açylmady.')}finally{if(msg.isConnected){msg.disabled=false;msg.setAttribute('aria-busy','false')}}return}}
